 ...
 */
 ...
 class MtpApiManage {
  function (Storage, MtpAuthorizer, MtpNetworkerFactory, MtpSingleInstanceService, AppRuntimeManager, ErrorService, qSync, $rootScope, $q, WebPushApiManager, TelegramMeWebService) {
...
class AppLoginController {
  constructor ($scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, LayoutSwitchService, WebPushApiManager, TelegramMeWebService, _) {
    ...
    function initPhoneCountry () { // NOTE: ???
    ...
    function selectPhoneCountryByIso2 (countryIso2) { // NOTE: ???
      if (countryIso2) {
        var i, country
    ...
    $scope.sendCode = function () { // NOTE: $scope можно поменять на простой объект, входящий  глобальный ($rootScope)
      $timeout.cancel(nextTimeout)
